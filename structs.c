...
int main() {
  struct rectangle house = {
      10, 20}; // non-standard initialization (compared to primitive types)
  position destination = {
      0, 22}; // standard initialization (type identifier = value)

  house.length = 11; // can change values using dot operator
  destination.x = 20;
  printf("destination: %d %d\n", destination.x, destination.y);

  buildingPlan myHouse = {"Arpan Abhishek", {10, 20}, {11, 34}};
  myHouse.position.x = 30; // Now you can traverse structure using dot operator

  printf("The house at %d %d (size %d %d) is owned by %s\n", myHouse.position.x,
         myHouse.position.y, myHouse.rectangle.length, myHouse.rectangle.width,
         myHouse.owner);
  ...
  buildingPlan myPlan = createPlan("Arpan", 10, 10, 30, 30);

  printf("The house at %d %d (size %d %d) is owned by %s\n", myPlan.position.x,
         myPlan.position.y, myPlan.rectangle.length, myPlan.rectangle.width,
         myPlan.owner);
